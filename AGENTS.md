...
- 出力は日本語で行う。
- 画面・Controller・モデルで新たに日本語文言を追加する場合は、原則としてRails i18nを利用し、`config/locales` 配下で管理する。画面固有の長文説明など、再利用しない文言は必要に応じて直接記述してよい。
- ファイルの削除は行わない。
...
- ブランチ名に`codex/`は使用しない。作業内容に応じて`feat/`、`fix/`、`refactor/`、`chore/`、`docs/`、`test/`などの適切な接頭語を選び、内容が分かる短い名前を付ける。
- `main`ブランチではファイルを編集しない。現在のブランチが`main`の場合、「同じブランチで進めて」と指示された場合でも、作業開始前に作業用ブランチを作成して切り替える。ただし、ユーザーが`main`上での変更を明示的に指示した場合のみ例外とする。
